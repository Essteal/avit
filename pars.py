    ...
    with session_scope() as session:
        ...
        if slave is None:
            ...
            await message.answer(
                '🔒 <i>Привет, для доступа к парсеру нужно иметь подписку, можешь приобрести ее у @хуй</i>',
                parse_mode=ParseMode.HTML)
            return
        if slave.sub is None:
            await message.answer(
                '🔒 <i>Для доступа к парсеру нужно иметь подписку, можешь приобрести ее у @хуй</i>',
                parse_mode=ParseMode.HTML)
            return
        sub_validate = False
